...
def build_model_1(domain_id):
    ...
    url=get_url_pre()+"/supersonic/api/semantic/model/getModelList/"+str(domain_id)
    ...
    if model_list is None :
        ...
        if not exist:
            build=True
    if build:
        url=get_url_pre()+"/supersonic/api/semantic/model/createModel"
        authorization=get_authorization()
        header = {}
...
def build_model_rela1(domain_id,from_model_id,to_model_id):
    ...
    url=get_url_pre()+"/supersonic/api/semantic/modelRela"
    authorization=get_authorization()
    header = {}
    header["Authorization"] =authorization
    resp=requests.post(url=url, headers=header,json=json_dict)
...
def build_dataSet(domain_id,model_id1,model_id2,model_id3,model_id4):
    url=get_url_pre()+"/supersonic/api/semantic/dimension/getDimensionList/"+str(model_id1)
    dimension_list1=get_id_list(get_list(url))
    url=get_url_pre()+"/supersonic/api/semantic/dimension/getDimensionList/"+str(model_id2)
    dimension_list2=get_id_list(get_list(url))
    url=get_url_pre()+"/supersonic/api/semantic/dimension/getDimensionList/"+str(model_id3)
    dimension_list3=get_id_list(get_list(url))
    url=get_url_pre()+"/supersonic/api/semantic/dimension/getDimensionList/"+str(model_id4)
    dimension_list4=get_id_list(get_list(url))

    url=get_url_pre()+"/supersonic/api/semantic/metric/getMetricList/"+str(model_id1)
    metric_list1=get_id_list(get_list(url))
    url=get_url_pre()+"/supersonic/api/semantic/metric/getMetricList/"+str(model_id2)
    metric_list2=get_id_list(get_list(url))
    url=get_url_pre()+"/supersonic/api/semantic/metric/getMetricList/"+str(model_id3)
    metric_list3=get_id_list(get_list(url))
    url=get_url_pre()+"/supersonic/api/semantic/metric/getMetricList/"+str(model_id4)
    ...
    url=get_url_pre()+"/supersonic/api/semantic/dataSet"
    authorization=get_authorization()
    header = {}
    header["Authorization"] =authorization
    resp=requests.post(url=url, headers=header,json=json_dict)

    url=get_url_pre()+"/supersonic/api/semantic/dataSet/getDataSetList?domainId="+str(domain_id)
    print(url)
    resp=get_list(url)
    data={}
    data["id"]=resp[0]["id"]
...
def build_agent(dataSetId):
    ...
    json_dict={"id":10,
               ...
               "dataSetIds":[dataSetId]}
    url=get_url_pre()+"/supersonic/api/chat/agent"
    authorization=get_authorization()
    header = {}
    header["Authorization"] =authorization
    resp=requests.post(url=url, headers=header,json=json_dict)
def build_chat(agentId):
    url=get_url_pre()+"/supersonic/api/chat/manage/save?chatName=DuSQL问答&agentId="+str(agentId)
    authorization=get_authorization()
    header = {}
    header["Authorization"] =authorization
    resp=requests.post(url=url, headers=header)

    url=get_url_pre()+"/supersonic/api/chat/manage/getAll?agentId="+str(agentId)
    data=get_list(url)
    return data[0]["chatId"]
def build_dim_value_dict(modelIds,info):
    url=get_url_pre()+"/supersonic/api/chat/dict/task"
    authorization=get_authorization()
    header = {}
    header["Authorization"] =authorization
    data={
