...
public class PostgresqlAdaptor extends BaseDbAdaptor {
    ...
    /**
     * 重写函数名修正器方法
     * 该方法用于将SQL语句中的特定函数名替换为正确的函数名，并根据函数名添加相应的参数
    ...
    /**
     * 获取指定数据库模式下的所有表和视图名称。
     * 该方法通过提供的连接信息连接到数据库，并使用数据库元数据查询指定模式下的所有表和视图。
    ...
    public List<String> getTables(ConnectInfo connectionInfo, String schemaName)
        ...
        try (ResultSet resultSet =
                metaData.getTables(null, getParameterValue(connectionInfo.getUrl(), "currentSchema"), null, new String[] {"TABLE", "VIEW"})) {
            // 遍历查询结果，提取表和视图的名称。
